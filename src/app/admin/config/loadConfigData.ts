...
export async function loadConfigData() {
  ...
  const { data: business } = await supabase
    .from("businesses")
    .select("id, name, slug, site_type, contact_whatsapp, contact_phone, contact_email, contact_site, website_url, address, about_business, differentials, differentials_cards, policies, customer_classes, customer_ages, payment_methods, service_modes, logo_url, logo_gallery, hero_gradient, hero_style, share_image_url, share_description, vitrine_cover_url, vitrine_cover_urls")
    .eq("id", businessId!)
    .single();
