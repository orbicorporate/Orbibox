...
export default async function BoxesPage() {
  ...
  const { data: business } = await supabase.from("businesses").select("id, slug").eq("owner_id", user!.id).limit(1).single();
  ...
  return (
    <div className="flex flex-col">
      ...
      <BoxesManager businessId={business!.id} initialBoxes={boxes ?? []} slug={business!.slug} />
    </div>
  );
}
