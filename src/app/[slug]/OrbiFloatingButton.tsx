...
export function OrbiFloatingButton({ onOpen, orbiColors, agentName }: { onOpen: () => void; orbiColors: string[] | null; agentName: string }) {
  ...
  return (
    <div className="pointer-events-none fixed bottom-5 right-4 z-[45] flex flex-col items-end gap-2">
      {showHint && (
        <button
          onClick={abrir}
          className="pointer-events-auto max-w-[220px] rounded-2xl rounded-br-md bg-surface-white px-3.5 py-2.5 text-left text-[13px] font-medium shadow-[0_6px_24px_rgba(0,0,0,0.16)] animate-[fadeInUp_0.3s_ease]"
        >
      ...
      <button
        ...
        className="pointer-events-auto flex h-14 w-14 items-center justify-center rounded-full bg-surface-white shadow-[0_6px_24px_rgba(0,0,0,0.22)] ring-1 ring-black/5 transition-transform active:scale-95"
      >
